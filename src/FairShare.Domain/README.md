...
- `Calculators/`
  ...
  - `OregonWorksheetCalculator` — the Oregon Child Support Worksheet (CSF 02 0910, OAR 137-050), lines 1a–9g including Children Attending School, medical support and the parenting-time credit. Takes `OregonWorksheetInput` (richer than `ParentData`) and returns `OregonCalculationOutcome` (both parents can owe at once); not yet wired into `IChildSupportCalculator`/the catalog. Golden cases come from the official DOJ workbook via `Generate-OregonGoldenCases.ps1`.
